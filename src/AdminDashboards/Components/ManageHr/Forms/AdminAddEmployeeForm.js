...
import {
  GetRegEmployee,
  UpdateEmployeeData,
} from "../../../../Store/Actions/Dashboard/EmployeeActions/EmployeeRegAction";
import { useAuth } from "../../../../Context/userAuthContext";
import { API_URL } from "../../../../config";
import Swal from "sweetalert2";
import { GetAllEmployeeAction } from "../../../../Store/Actions/Dashboard/EmployeeActions/GetAllEmployee";

const AdminAddEmployeeForm = ({ ToggleAddEmployee, update, data }) => {
  ...
  const finalUpdateForm = (formdata, { resetForm }) => {
    const newFormData = {
      ...formdata,
      role: slectedDesignation,
      departName: department,
    ...
    dispatch(UpdateEmployeeData(data?._id, newFormData)).then(() => {
      ToggleAddEmployee();
      resetForm();
      dispatch(GetAllEmployeeAction()).then(() => {
  ...
  useEffect(() => {
    if (data) {
      const allDataForm = Object.keys(dataForm);
      allDataForm.forEach((element) => {
        dataForm[element] = data[element];
      });
      setDepartment(data?.departName);
      setSelectedDesignation(data?.role);
      setUploadedImg({
        img: data?.img,
        documentOneImg: data?.documentOneImg,
        documentTwoImg: data?.documentTwoImg,
        documentThreeImg: data?.documentThreeImg,
      });
    }
  }, [data]);
        ...
        <Card className=" p-4 border-0">
          <div className="AddServiceMan_Main">
            <Formik
              ...
              onSubmit={!update ? finalFormSubmit : finalUpdateForm}
            >
              {({ values, handleChange, handleSubmit }) => (
                <Form onSubmit={handleSubmit}>
                  ...
                  <Row>
                    ...
                    <Col md={6} className={update && "row"}>
                      {update && (
                        <img
                          className={update && "col-6"}
                          src={API_URL + "/uploads/" + uploadedImg?.img}
                          width={200}
                          height={100}
                          alt="uploadedImg"
                        />
                      )}
                      <FormGroup className={update && "col-6"}>
                    ...
                    <Col md={6} className={update && "row"}>
                      {update && (
                        <img
                          src={
                            API_URL + "/uploads/" + uploadedImg?.documentOneImg
                          }
                          width={200}
                          className={update && "col-6"}
                          height={100}
                    ...
                    <Col md={6} className={update && "row"}>
                      {update && (
                        <img
                          className={update && "col-6"}
                          src={
                            API_URL + "/uploads/" + uploadedImg?.documentTwoImg
                          }
                          width={200}
                          height={100}
                          alt="uploadedImg"
